...
def compute_global_frequencies(dictionary: List[str]) -> Dict[str, float]:
    frequencies = dict.fromkeys(list("ABCDEFGHIJKLMNOPQRSTUVXYZ"))
...
def compute_word_appearance_frequencies(dictionary: List[str]) -> Dict[str, float]:
    return compute_global_frequencies(list(map(lambda word: "".join(set(word)), dictionary)))


def pretty_print_frequencies(frequencies: Dict[str, float]) -> None:
    for letter, frequency in sorted(frequencies.items(), key=lambda item: item[1], reverse=True):
...
def main(dictionary_path: str, word_length: int) -> None:
    ...
    click.secho(f"Words of {word_length} letters in dictionary: {len(filtered_dictionary)}", bold=True, fg="green")
    click.secho(f"Letter global frequencies:", bold=True, fg="green")
    pretty_print_frequencies(compute_global_frequencies(filtered_dictionary))
    click.secho(f"Letter word appearance frequencies:", bold=True, fg="green")
